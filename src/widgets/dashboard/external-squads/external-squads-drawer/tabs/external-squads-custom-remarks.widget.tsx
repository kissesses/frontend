...
import { GetExternalSquadByUuidCommand } from '@kissesses/backend-contract'
import { useCallback, useEffect, useState } from 'react'
import { useTranslation } from 'react-i18next'
import { PiClockCountdown, PiClockUser, PiListChecks, PiProhibit } from 'react-icons/pi'
import { TbDeviceFloppy, TbDevices2, TbListLetters, TbShieldLock, TbX } from 'react-icons/tb'
...
export const ExternalSquadsCustomRemarksTabWidget = (props: IProps) => {
    ...
    const [remarks, setRemarks] = useState<Record<string, string[]>>({
        ...
        HWIDBlocked: DEFAULT_REMARKS.HWIDBlocked
    })
        ...
        if (isOpen && externalSquad) {
            ...
            if (currentRemarks) {
                ...
                setRemarks({
                    ...
                    HWIDBlocked: processRemarks(currentRemarks.HWIDBlocked)
                })
            } else {
    ...
    const updateHWIDBlockedRemarks = useCallback((newRemarks: string[]) => {
    ...
    const handleUpdateExternalSquad = () => {
        ...
        const HWIDBlockedFiltered = filterEmptyStrings(remarks.HWIDBlocked)
                ...
                >
                    {(styles) => (
                        <Stack gap="md" style={styles}>
                            ...
                            <RemarksManager
                                icon={<TbShieldLock size="24px" />}
                                iconColor="red"
                                initialRemarks={remarks.HWIDBlocked}
                                onChange={updateHWIDBlockedRemarks}
                                title={t('subscription-user-remarks-card.widget.hwid-blocked')}
                            />
