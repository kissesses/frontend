...
export const OAuth2LoginButtonsFeature = (props: IProps) => {
    const { authentication, loadingProvider: externalLoadingProvider, onProviderClick } = props
    const [internalLoadingProvider, setInternalLoadingProvider] =
        useState<null | TOAuth2ProvidersKeys>(null)
    const loadingProvider = externalLoadingProvider ?? internalLoadingProvider
    ...
    const { mutate: oauth2Authorize } = useOAuth2Authorize({
        mutationFns: {
            onSuccess: (data) => {
                ...
                setTimeout(() => {
                    setInternalLoadingProvider(null)
                }, 1000)
            },
            onError: () => {
                setInternalLoadingProvider(null)
    ...
    const handleOAuth2Login = async (provider: TOAuth2ProvidersKeys) => {
        if (onProviderClick) {
            await onProviderClick(provider)
            return
        }

        setInternalLoadingProvider(provider)
        oauth2Authorize({
            variables: {
                provider
            }
