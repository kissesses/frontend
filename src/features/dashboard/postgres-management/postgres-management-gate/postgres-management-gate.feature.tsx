import { PostgresManagementOAuthButtonsFeature } from '@features/dashboard/postgres-management/postgres-management-oauth/postgres-management-oauth.feature'
import {
    Alert,
    Button,
    Center,
...
import { GetPostgresManagementGateStatusCommand } from '@kissesses/backend-contract'
import {
...
import {
    useRequestPostgresManagementCode,
    useVerifyPostgresManagementCode,
    useVerifyPostgresManagementPasskey,
    useVerifyPostgresManagementPassword
} from '@shared/api/hooks/postgres-management/postgres-management.mutation.hooks'
import { usePostgresManagementPasskeyOptions } from '@shared/api/hooks/postgres-management/postgres-management.query.hooks'

interface IProps {
    gateStatus: GetPostgresManagementGateStatusCommand.Response['response']
    onElevated: () => void
}

export const PostgresManagementGateFeature = (props: IProps) => {
    ...
    const { mutateAsync: verifyPasskey, isPending: isVerifyingPasskey } =
        useVerifyPostgresManagementPasskey()
    const { refetch: refetchPasskeyOptions } = usePostgresManagementPasskeyOptions()

    const invalidateGate = () => {
        queryClient.invalidateQueries({
            queryKey: QueryKeys.postgresManagement.gateStatus.queryKey
        })
    }
    ...
    const handlePasskey = async () => {
        ...
        try {
            ...
            if (error instanceof Error && error.name === 'NotAllowedError') {
                notifications.show({
                    title: t('postgres-management.gate.passkey-cancelled'),
                    message: t('postgres-management.gate.passkey-cancelled-message'),
                    color: 'yellow'
                })
            }
        } finally {
    ...
    return (
        <Center mih="50vh" px="md">
            <Paper maw={440} p="xl" radius="md" w="100%" withBorder>
                <Stack gap="lg">
                    <Stack gap="xs">
                        <Title order={3}>{t('postgres-management.gate.title')}</Title>
                        <Text c="dimmed" size="sm">
                            {authMode === 'telegram'
                                ? t('postgres-management.gate.telegram-description')
                                : t('postgres-management.gate.alternative-description')}
                        </Text>
                    </Stack>
                    ...
                    {authMode === 'telegram' ? (
                        <Stack gap="md">
                            <Button
                                ...
                                variant="light"
                            >
                                {t('postgres-management.gate.send-code')}
                            </Button>

                            <Stack gap="xs">
                                <Text fw={500} size="sm">
                                    {t('postgres-management.gate.enter-code')}
                                </Text>
                                ...
                                >
                                    {t('postgres-management.gate.verify')}
                                </Button>
                            </Stack>
                        </Stack>
                    ) : (
                        <Stack gap="md">
                            {alternativeMethods.passkey && (
                                <Button
                                    ...
                                    variant="light"
                                >
                                    {t('postgres-management.gate.passkey')}
                                </Button>
                            )}

                            {hasOAuth2 && (
                                <PostgresManagementOAuthButtonsFeature
                            ...
                            {!hasAlternativePrimary && !alternativeMethods.password && (
                                <Alert color="yellow" variant="light">
                                    {t('postgres-management.gate.no-methods')}
                                </Alert>
                            )}
                        </Stack>
                    )}
                    ...
                    {showPasswordFallback && (
                        <>
                            {hasAlternativePrimary && (
                                <Divider label={t('postgres-management.gate.or-password')} />
                            )}
                            <Stack gap="sm">
                                <PasswordInput
                                    label={t('postgres-management.gate.password-label')}
                                ...
                                >
                                    {t('postgres-management.gate.confirm-password')}
                                </Button>
                            </Stack>
                        </>
                    )}
